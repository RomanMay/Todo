...
$(() => {
	...
	function generateTaskView(task) {
		return `<div class="task_container" id="${task.id}">
					...
					<p class="task_text" ${task.isCompleted === true ? css() : class { "noneDecoration" }}>${task.text}</p>          
                </div>  `
	}
	...
	function css() {
		$('.task_text').css("text-decoration", "line_through")
	}
	function nocss() {
		$('.task_text').css("text-decoration", "none")
	}
