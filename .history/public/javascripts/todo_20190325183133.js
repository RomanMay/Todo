...
$(() => {
	...
	function generateTaskView(task) {
		return `<div class="task_container" id="${task.id}">
					...
					<p class="task_text" ${task.isCompleted === true ? css() : nocss()}}>${task.text}</p>          
                </div>  `
	}
	...
	function css() {
		return $('.task_text').css("text-decoration", "line_through")
	}
	function nocss() {
		return $('.task_text').css("text-decoration", "none")
	}
